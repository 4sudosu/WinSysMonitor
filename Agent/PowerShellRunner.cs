...
public static class PowerShellRunner
{
    public static (string? Output, string? Error) RunInInteractiveSession(string outFile, int timeoutSec = 15)
    {
        ...
        var taskName = $"WinSysMonitorCapture{id}";
        ...
        try
        {
            ...
            // XML definition (NOT PowerShell). It runs THIS agent EXE with
            // --capture so no powershell.exe ever appears for the capture.
            if (!TryInteractiveUser(out var user))
                return (null, "No interactive user is logged on");
            var taskXml = Path.Combine(dir, $"t-{id}.xml");
            File.WriteAllText(taskXml, BuildTaskXml(user, taskName, outFile), Encoding.Unicode);
    ...
    {
        var dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Windows), "Temp", "WinSysMonitor");
        Directory.CreateDirectory(dir);
        try
        {
    ...
    /// "Queued" forever).
    /// </summary>
    private static string BuildTaskXml(string user, string taskName, string outFile)
    {
        ...
        return "<?xml version=\"1.0\" encoding=\"UTF-16\"?>\r\n" +
            ...
            "  <RegistrationInfo>\r\n" +
            $"    <Description>WinSysMonitor interactive screen capture</Description>\r\n" +
            $"    <URI>\\{E(taskName)}</URI>\r\n" +
            "  </RegistrationInfo>\r\n" +
            "  <Triggers />\r\n" +
            "  <Principals>\r\n" +
            ...
            $"      <Arguments>--capture \"{E(outFile)}\"</Arguments>\r\n" +
            "    </Exec>\r\n" +
            "  </Actions>\r\n" +
            "</Task>\r\n";
    }
