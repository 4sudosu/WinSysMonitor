; WinSysMonitor — Standalone Installer (admin-only, installs a 24x7 Windows service)
; Build: compile with ISCC.exe after publishing a self-contained single-file
; agent EXE into ..\publish-single\WinSysMonitor.exe
;
; This installer:
;   1. Requires Administrator (PrivilegesRequired=admin)
;   2. Installs the agent into Program Files for all users
;   3. Registers and starts the "WinSysMonitor" Windows service (runs as
...
#define MyAppName "WinSysMonitor"
#ifndef MyAppVersion
#define MyAppVersion "0.0.0.1"
#endif
#define MyAppExeName "WinSysMonitor.exe"
#define MyAppServiceName "WinSysMonitor"
#ifndef MyOutputBaseFilename
#define MyOutputBaseFilename "WinSysMonitor-Setup"
#endif
...
AppPublisher=BreakFix IT
DefaultDirName={autopf}\WinSysMonitor
...
[Files]
Source: "..\publish-single\WinSysMonitor.exe"; DestDir: "{app}"; Flags: ignoreversion; BeforeInstall: StopExistingService
...
Type: files; Name: "{app}\WinSysMonitor.new.exe"
...
// Stop + delete any previous WinSysMonitor service before overwriting the EXE,
...
begin
  ServerPage := CreateInputQueryPage(
    wpSelectTasks,
    'WinSysMonitor Server',
    'Where should the agent connect?',
    'Enter the IP address or hostname of the WinSysMonitor server, then the port and the agent token. These can be changed later by an administrator editing agent.config.json in the install folder (the agent reloads it automatically).'
  );
  ...
  if ServerPage.Values[2] = '' then
    ServerPage.Values[2] := 'WINSYS_MONITOR_TOKEN_0001';
end;
...
begin
  ...
  begin
    ...
    Script := '$ErrorActionPreference = ''Continue''' + #13#10 +
      ...
      '}' + #13#10 +
      'New-Service -Name $svc -BinaryPathName $binPath -StartupType Automatic -DisplayName ''Windows System Monitor'' | Out-Null' + #13#10 +
    ...
    ScriptPath := ExpandConstant('{tmp}\install-winsysmonitor-service.ps1');
    ...
    if ResultCode <> 0 then
      MsgBox('The WinSysMonitor service could not be created/started (error code ' +
